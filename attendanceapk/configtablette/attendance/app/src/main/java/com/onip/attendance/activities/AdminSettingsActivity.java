...
/**
 * Paramètres administrateur : serveur RH et rechargement des données.
 */
public class AdminSettingsActivity extends Activity {
    ...
    private void testConnection(boolean startReloadAfterSuccess) {
        ...
        dataManager.testServerConnection(new DataManager.ConnectionCallback() {
            @Override
            public void onSuccess(int enrolledCount, int employeeCount) {
                runOnUiThread(() -> {
                    setBusy(false);
