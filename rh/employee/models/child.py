...
class Child(Base):
    ...
    full_name = models.CharField(
        verbose_name=_('nom complet'), max_length=100, blank=True, null=True, default=None
    )

    date_of_birth = DateField(verbose_name=_('date de naissance'), blank=True, null=True, default=None)
    birth_certificate = models.FileField(verbose_name=_('certificat de naissance'), upload_to=upload_directory_file, blank=True, null=True, default=None)

    search_fields = ('employee__registration_number', 'employee__first_name', 'employee__middle_name', 'employee__last_name', 'full_name') 
    inline_form_fields = ('full_name', 'date_of_birth')
