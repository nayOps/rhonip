...
class Change(BaseView):
    ...
    def get(self, request, app, model, pk):
        if app == 'employee' and model == 'employee':
            query = request.META.get('QUERY_STRING', '')
            url = reverse_lazy('employee:change', kwargs={'pk': pk})
            if query:
                url = f'{url}?{query}'
            return redirect(url)
        ...
        # Cas spécial pour Employee: un employé normal ne peut modifier que son propre profil
        ...
        if app == 'employee' and model == 'employee':
            ...
            return Employee.as_view()(request, pk=pk)
